...
- Tags: PR reference and SHA

**For Main Branch Pushes:**
- Generates a version tag with timestamp format (`YYYY.MM.DD.HHMMSS`)
- Creates and pushes a Git tag to the repository
- Builds the Docker image
- Logs in to Azure Container Registry
- Pushes the image with multiple tags:
  - Version timestamp (e.g., `2026.01.21.143025`)
  - `latest` (always for main branch)
  - Git SHA (e.g., `sha-abc123`) for PRs
...
- **Automatic versioning** with timestamp-based tags (`YYYY.MM.DD.HHMMSS`)
- **Git tag creation** automatically pushed to GitHub on main branch
- **Automatic tagging** based on version timestamp, latest, PR, and SHA
...
2. **Branch Strategy**: The workflow is optimized for `main` branch with automatic versioning and tagging
3. **Pull Requests**: Always test in PRs before merging - Docker images are built but not pushed to registry
...
11. **Version Tags**: Automatic timestamp-based versioning ensures unique, chronological version tracking
12. **Git Tags**: All main branch builds create corresponding Git tags for easy reference and rollback
...
- `{YYYY.MM.DD.HHMMSS}` - Timestamp version (e.g., `2026.01.21.143025`)
- `latest` - Always points to the most recent main build

**For Pull Requests:**
- `pr-{number}` - PR reference (e.g., `pr-42`)
- `sha-{git-sha}` - Commit SHA (e.g., `sha-abc123`)

### Git Tags
**For Main Branch (Push):**
- A Git tag is automatically created and pushed to the repository with the format `YYYY.MM.DD.HHMMSS`
- This tag can be used for releases, rollbacks, and version tracking
- Example: `2026.01.21.143025`
